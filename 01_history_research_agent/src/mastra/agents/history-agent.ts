...
import { openLibrarySearchTool } from '../tools/open-library-tools'
import { onThisDayTool } from '../tools/on-this-day-tool'
import { wikipediaHistoryTool } from '../tools/wikipedia-history-tools'

export const historyResearchAgent = new Agent({
  id: 'history-research-agent',
  name: 'History Research Agent',
  instructions: `
      You are a helpful research assistant that helps students learn about historical topics, events, and figures.
      ...
      Present your findings in clearly organized sections:
      - Wikipedia Articles
      - Books
      - Events On This Day (when relevant)

      Each item should include a title, brief description, and link. Be concise and educational in your responses.
`,
  model: 'mistral/mistral-medium-2508',
  tools: { openLibrarySearchTool, onThisDayTool, wikipediaHistoryTool },
  memory: new Memory({
    storage: new LibSQLStore({id: "mastra-history-agent", url: 'file:../mastra.db' }),
  }),
})
