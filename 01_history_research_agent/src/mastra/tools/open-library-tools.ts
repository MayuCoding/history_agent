/**
 *  This project tests your ability to create a Mastra tool that calls an
...
export const openLibrarySearchTool = createTool({
    ...
    inputSchema: z.object({
        topic: z.string().describe('Historical topic or query to search on Open Library'),
        limit: z.number().min(1).max(10).optional().default(5),
    }),
    ...
    execute: async ({ context }) => {
        ...
        const books = (data.docs ?? []).map((doc) => ({
            ...
            firstPublished: doc.first_publish_year ?? undefined,
            source: 'openlibrary' as const,
        }));
        return { books };
    },
