...
export const onThisDayTool = createTool({
    ...
    execute: async ({ context }) => {
        ...
        const dd = String(context.day).padStart(2, '0');
        const limit = context.limit ?? 5; 
        
        // url from where we get the results
        const url = `https://api.wikimedia.org/feed/v1/wikipedia/en/onthisday/events/${mm}/${dd}`;
        const response = await fetch(url, { headers: { 'User-Agent': 'mastra-history-agent/1.0' } });

        if (!response.ok) { // 200 - Ok/Successful, 404 - Not Found
            throw new Error(`Wikimedia On This Day request failed with status ${response.status}`);
        }

        const data = (await response.json()) as OnThisDayResponse;
        // extract events from response data
        const events = (data.events ?? []).slice(0, limit).map((event) => {
